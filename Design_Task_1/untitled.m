...
Hdq = dfilt.dffir;
quantized_len = 14;
Hdq.Numerator = fi(Hd.Numerator, 1 ,quantized_len, quantized_len);
Ntot_a = length(b) * quantized_len;

fprintf('Filter with more lenient specs:\n');
fprintf('N_tot: %d\n', Ntot_a);
...
Ntot_b = length(b) * quantized_len;

fprintf('Filter with stricter specs:\n');
fprintf('N_tot: %d\n', Ntot_b);
fprintf('There are %d coeffients in the filter\n', length(b))
fprintf('The sp.n format of the coefficients is s%d.%d\n', quantized_len, quantized_len)
fvtool(Hd, Hdq2);
